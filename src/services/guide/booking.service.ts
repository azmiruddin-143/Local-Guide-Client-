...
export async function getMyBookings(guideId: string, queryString?: string) {
    try {
        const response = await serverFetch.get(`/bookings/guide/${guideId}${queryString ? `?${queryString}` : ""}`);
...
export async function confirmBooking(id: string) {
    try {
        ...
        const result = await response.json();
        revalidatePath("/guide/dashboard/bookings");
        return result;
    } catch (error: any) {
        console.log(error);
        return {
