...
export default function CategoryBreakdown({ categories }: CategoryBreakdownProps) {
  const totalBookings = categories.reduce((sum, cat) => sum + cat.count, 0);
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-600">Your tour preferences</p>
        </div>
      ...
      {categories.length === 0 ? (
        ...
        <div className="space-y-4">
          {categories.map((category, index) => {
            const percentage = ((category.count / totalBookings) * 100).toFixed(1);
              ...
              <div key={index} className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <span className="font-semibold text-gray-900">{category.count} tours</span>
                ...
                <p className="text-xs text-gray-500 pl-6">৳{category.totalSpent.toLocaleString()} spent</p>
              </div>
            );
          })}
        </div>
