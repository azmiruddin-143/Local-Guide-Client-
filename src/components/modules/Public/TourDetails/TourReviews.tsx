...
import { getReviewsBytourId } from "@/services/review/review.service";
...
export default function TourReviews({ tourId }: TourReviewsProps) {
  ...
  const loadReviews = async () => {
    setLoading(true);
    try {
      const response: ReviewsResponse = await getReviewsBytourId(tourId, page, limit);
  ...
  if (pagination.total === 0 && !loading) {
    return (
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
        <h2 className="text-2xl font-bold text-gray-900 mb-6">Tour Reviews</h2>
