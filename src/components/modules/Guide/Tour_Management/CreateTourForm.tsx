...
export default function CreateTourForm({user}: {user: IUser}) {
  ...
  return (
    <form action={handleSubmit} className={`space-y-8 ${user.isVerified ? '': 'hidden'}`}>
      {/* Basic Information */}
      <div className="bg-white rounded-lg border p-3 md:p-6 space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2">
              {popularLanguages.map((language) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-md border transition-colors ${selectedLanguages.includes(language)
      ...
      <div className="bg-white rounded-lg border p-6 space-y-6">
        ...
        <Field>
          ...
          <p className="text-xs text-gray-500 mt-1">
            Upload high-quality images that showcase your tour. First image will
            be the cover photo.
          </p>
        </Field>
      </div>
      ...
      <div className="flex  items-center  justify-end gap-4 pt-6 border-t">
        ...
        <Button
          ...
          className=" md:flex-1 h-9 text-base max-w-[300px] font-semibold"
        >
          {isPending ? (
